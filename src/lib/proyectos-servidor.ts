...
export async function cargarProyecto(id: string, facilitador: Facilitador) {
  ...
  const [medProc, accProc, retos, carreras, retos5s, casos] = idsProcesos.length
    ? await Promise.all([
        ...
        sb.from('ml_sesiones').select('proceso_id').in('proceso_id', idsProcesos),
      ])
  ...
  const vistaProcesos: ProcesoDelProyecto[] = listaProcesos.map((x) => {
    ...
    return {
      ...
      juegos: [...((retos.data ?? []) as any[]), ...((carreras.data ?? []) as any[]), ...((retos5s.data ?? []) as any[]), ...((casos.data ?? []) as any[])].filter((j) => j.proceso_id === x.id).length,
    };
  });
